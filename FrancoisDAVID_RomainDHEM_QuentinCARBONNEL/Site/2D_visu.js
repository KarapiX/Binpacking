document.addEventListener("DOMContentLoaded", function() {
    ...
    function createWagon2D(wagonData, wagonNumber) {
        ...
        let colorIndex = 0;
        let totalArea = 0;
        const scale = 1; // Scale factor to reduce size
    ...
    function loadAndDisplayJSON(url, container, key) {
        fetch(url)
            ...
            .then(jsonData => {
                if (jsonData.d2 && jsonData.d2[key] && jsonData.d2[key].structure) {
                    ...
                    Object.keys(wagonsData).forEach((wagonKey, index) => {
                        const wagon = createWagon2D(wagonsData[wagonKey], index + 1);
    ...
    loadAndDisplayJSON('2D_online_francois.json', onlineContainer, 'Online_FF');
    loadAndDisplayJSON('2D_offline_francois.json', offlineContainer, 'Offline_FF');
    ...
    function MouseWheelHandler(e, container) {
        ...
        if (delta) {
            e.preventDefault();
            e.stopPropagation();
            scrollAmount -= delta * 120; // Adjust scrolling speed as needed
            if (!isScrolling) {
                isScrolling = true;
